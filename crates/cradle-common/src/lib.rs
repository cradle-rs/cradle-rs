...
/// encap; the inner destination MAC a received routed frame carries). For an
/// E-Line VNI (`VNI_F_ELINE`, VPWS over VXLAN) the inner frame is emitted
/// raw on the AC — `vrf_id` is the AC ifindex, or with `VNI_F_ELINE_VLAN`
/// the `DX2V` table the inner 802.1Q VID selects the AC from.
#[repr(C)]
#[derive(Clone, Copy, Debug)]
pub struct VniInfo {
...
/// [`VniInfo`] is an E-Line VNI (EVPN VPWS over VXLAN, RFC 8365 §6): the
/// inner frame is emitted raw on the attachment circuit `vrf_id` names —
/// no FDB, no learning (the VXLAN analog of `End.DX2`).
pub const VNI_F_ELINE: u16 = 1 << 2;
/// With [`VNI_F_ELINE`]: the E-Line is VLAN-scoped — `vrf_id` is a `DX2V`
/// VLAN-table id and the inner frame's 802.1Q VID picks the AC from it
/// (the VXLAN analog of `End.DX2V`).
pub const VNI_F_ELINE_VLAN: u16 = 1 << 3;
...
pub struct ReplTarget {
    ...
    /// semantic of the map. Besides `REPL_SID`, this is also the value shape
    /// of the VPWS `XCONNECT`/`XCONNECT_VLAN` maps — the E-Line remote
    /// endpoint is the same (kind, vni, addr) triple, unicast instead of a
    /// flood copy.
    pub kind: u32,
...
/// EVPN VPWS over VXLAN: an E-Line VNI's frame stripped and emitted raw
