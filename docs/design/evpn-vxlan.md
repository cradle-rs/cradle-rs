...
| **Type-4 (ES) / AR** | multihoming, assisted replication | split-horizon, DF election | DF election computed and teed; the non-DF filter (`ES_DF`), the split-horizon / local-bias filter (`VTEP_ES` × `PORT_ES`) and the aliasing nexthop group (`ES_NHG`, `FdbRemote.esi`) are in cradle — see bgp-evpn.md §Multihoming; the zebra consumer for aliasing is next |
