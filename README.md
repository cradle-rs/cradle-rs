...
| Dual-stack pods (IPv6 IPAM) | ✅ | node-local v6 allocator + ptp v6 gateway (`fe80::1`); pod `/128` into FIB6; `cradle_cni_v6` BDD ([design](docs/design/kube-proxy-dualstack.md)). Policy engine is dual-stack too |
...
| NetworkPolicy / identity enforcement | ✅ | ingress **and egress**, dual-stack, ipBlock CIDRs (with `except`), named ports: ingress verdict at delivery (`cradle_egress`), egress at the pod's veth hook (stateful both ways, default-allow); `cradle-k8s --enforce-policy` translates k8s NetworkPolicies; `cradle_policy`/`cradle_policy_v6` BDD + kind e2e ([design](docs/design/policy.md)) |
...
| TTL propagation (pipe/uniform) | ✅ | per-LSP pipe/uniform selectable (RFC 3443): pipe imposition seeds the label TTL 255 (`NH_F_MPLS_PIPE`), uniform disposition writes the popped label TTL back into the inner IP header (per-ILM, IPv4 csum fixup); pipe is the default; driven by static gRPC/JSON config and zebra-rs `set mpls ttl propagate {pipe\|uniform}` with a per-VRF `vrf <name> mpls ttl propagate` override; `cradle_mpls_ttl` BDD. The RFC 3443 forwarded/local split is complete: `propagate` (above) is the forwarded half (cradle imposes only transit); the local half is `mpls ttl propagate-local {pipe\|uniform}`, which drives the host kernel's `net.mpls.ip_ttl_propagate` (locally-originated traffic is imposed by the kernel's lwtunnel route, never cradle) — `mpls_ttl_propagate_local` zebra-rs BDD. See [design](docs/design/mpls-ttl-propagation.md) |
| ICMP tunneling (LSP-tunneled TTL-exceeded) | ⬜ | RFC 3032 §3.4 / Junos `icmp-tunneling`, the companion to pipe-mode TTL propagation. Under pipe a transit LSR's TTL-exceeded has no route back to the hidden customer source — cradle punts `TTL<=1` to the host, whose ICMP is then unroutable — so pipe both hides the core *and* swallows `traceroute` errors. Not implemented: forwarding the ICMP down the LSP to the egress PE, which returns it toward the source, would restore `traceroute` under pipe. Under uniform (LSP exposed) transit TTL-exceeded is already delivered normally, so this only matters for pipe. See [design](docs/design/mpls-ttl-propagation.md#companion-features-out-of-scope-noted) |
...
| BGP color steering to a BSID | SR-MPLS / SRv6 | ✅ | `bgp sr-policy steering-mode {segment-list\|binding-sid}` (RFC 9256 §8.5): binding-sid steers a colored service route onto the policy's SR-MPLS BSID label (the ILM expands it) or SRv6 End.B6.Encaps BSID (H.Encap), with CO-bit endpoint fallback; segment-list (raw stack) stays the default. Both are teed end to end to cradle's eBPF FIB — incl. a service route received before its policy, re-steered on BSID activation: SR-MPLS `cradle_color_bsid_zebra`, SRv6 End.B6 `cradle_color_bsid_srv6_zebra` |
