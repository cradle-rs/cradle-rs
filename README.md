...
| BGP color steering to a BSID | SR-MPLS / SRv6 | 🔶 | `bgp sr-policy steering-mode {segment-list\|binding-sid}` (RFC 9256 §8.5): binding-sid steers a colored service route onto the policy's SR-MPLS BSID label (the ILM expands it) or SRv6 End.B6.Encaps BSID (H.Encap), with CO-bit endpoint fallback; segment-list (raw stack) stays the default. The SR-MPLS BSID color-steer is teed end to end to cradle's eBPF FIB — incl. a service route received before its policy, re-steered on BSID activation (`cradle_color_bsid_zebra`); the SRv6 End.B6 color-steer datapath is proven only at the zebra-rs FIB (`bgp_sr_policy_bsid_forwarding`), no dedicated cradle BDD yet |
