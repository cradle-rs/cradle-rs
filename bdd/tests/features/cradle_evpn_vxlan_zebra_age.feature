@serial
@cradle_evpn_vxlan_zebra_age
Feature: FDB aging expires idle MACs and withdraws their EVPN/VXLAN Type-2s
  ...
  The aging + WatchFdb age channel + the MacDel/fdb_del tee are all
  encap-agnostic, so this exercises the same path the SRv6 variant does but
  over the VXLAN tee: a user-space sweep expires local entries idle past
  `fdb_age_secs` (5s here) and bumps `fdb_aged`; the age event re-emits as an
  FdbDel that withdraws the Type-2 and removes the remote VXLAN FDB entry on
  the far PE. A later frame re-learns, re-advertises, and reconverges.

  Topology: the 2-PE dynamic EVPN/VXLAN (same as cradle_evpn_vxlan_zebra)
  with a 5s age. IPv6 is disabled on the CE NICs so the links are genuinely
  idle between pings (no ND/MLD chatter refreshing the entries).
  ```
   c1 ── pe1[cradle+zebra] ──10.0.12.0/24── pe2[cradle+zebra] ── c2
    bd 100 / VNI 100, fdb_age_secs 5, VTEP 192.0.2.1|192.0.2.2   bd 100 / VNI 100
  ```
