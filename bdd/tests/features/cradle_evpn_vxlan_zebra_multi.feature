@serial
@cradle_evpn_vxlan_zebra_multi
Feature: BGP EVPN over VXLAN drives multi-PE BUM replication in eBPF
  Three PEs in one bridge domain, everything BGP-driven: each PE's Type-3
  IMET advertises its VTEP, and the FibHandle tee turns every received
  Type-3 into a cradle VXLAN replication slot — a per-remote veth pair in
  the flood list whose far end per-copy VXLAN-encapsulates BUM toward that
  PE's VTEP. Unicast rides tee-installed VXLAN FDB entries from Type-2
  routes originated off the WatchFdb MAC-learn stream. Fully dynamic: no
  static ARP, no static FDB, no static replication config — cradle creates
  the slot veths itself.

  Topology (kernel v4+v6 forwarding off on the PEs; pe1 is the underlay
  hub, so pe2↔pe3 BGP and BUM transit pe1 as plain routed IPv4). VTEPs are
  loopback /32s reached over the directly-connected underlay by static
  routes:
  ```
        c1 ── pe1[cradle+zebra] ──10.0.12.0/24── pe2[cradle+zebra] ── c2
   bd 100         │  VTEP 192.0.2.1              VTEP 192.0.2.2
                  └────10.0.13.0/24── pe3[cradle+zebra] ── c3
                                       VTEP 192.0.2.3
  ```
  iBGP EVPN full mesh over the loopback VTEPs; per-PE kernel bridge+vxlan100
  is zebra's VNI declaration only. Every c*↔c* pair reaching each other
  proves the Type-3 → slot tee, per-copy replication, decap+flood, split
  horizon, and the learned-MAC Type-2 loop — across three PEs.

  Scenario: Bridge three CEs across a BGP-EVPN-over-VXLAN eBPF data plane
