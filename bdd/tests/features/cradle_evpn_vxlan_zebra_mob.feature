@serial
@cradle_evpn_vxlan_zebra_mob
Feature: MAC mobility moves a station between PEs over VXLAN (RFC 7432 §7.7)
  As an operator running a dynamic EVPN-over-VXLAN L2VPN on cradle
  ...
  The move machinery is encap-agnostic, so this exercises it over the VXLAN
  tee: at the NEW PE the datapath learn overwrites the remote FDB entry with
  a local one → WatchFdb reports it → zebra originates the Type-2 with the
  MAC Mobility extended community (seq = max remote + 1), preferred over the
  stale route. At the OLD PE, installing the new remote entry flips its local
  entry → the disappearance withdraws the old Type-2. A remote withdraw never
  clobbers a fresh local learn (fdb_remote_del is local-guarded, and the
  VXLAN fdb_add_vxlan carries the same displaced-local move hint).

  Topology: the 2-PE dynamic EVPN/VXLAN with a second CE port on pe2 hosting
  the mobile station's new location. Station M = 02:00:00:00:c1:01 /
  10.0.0.1 starts at c1 (behind pe1) and "migrates" to cm (behind pe2),
  then back.
  ```
   c1 ── pe1[cradle+zebra] ──10.0.12.0/24── pe2[cradle+zebra] ──┬── c2
    M, 10.0.0.1     VTEP 192.0.2.1      VTEP 192.0.2.2          └── cm (M's new home)
  ```
