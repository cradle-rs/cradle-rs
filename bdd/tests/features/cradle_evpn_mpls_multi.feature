...
Feature: EVPN-over-MPLS multi-PE BUM ingress replication in eBPF
  As an operator running an EVPN L2VPN across more than two PEs over MPLS
  I want a BUM frame replicated once per remote PE, each copy imposed with that
  PE's BUM service label, plus the local flood
  ...
  TC can't resize non-IP frames): each remote PE gets a "replication slot" — a
  veth pair whose A end joins the bridge domain's flood list. The TC flood
  clones the bare BUM frame into each slot; the copy arrives as ingress on the
  B end, where the XDP stage imposes that slot's `[transport][BUM label]` and
  forwards it out the underlay (FIB4 lookup on the remote PE). Overlay-received
  frames flood local-only (split horizon: never back into a slot).

  Topology (kernel MPLS untouched and kernel IPv4 forwarding off on all PEs;
  pe1 is the underlay hub, so pe2↔pe3 BUM **transits pe1 as MPLS** — pe1
  label-switches it rather than routing it):
  ```
        c1 ── pe1[cradle] ──10.0.12.0/24── pe2[cradle] ── c2
   bd 100        │  BUM label 1200          BUM label 2200
                 └────10.0.13.0/24── pe3[cradle] ── c3
                                      BUM label 3200
  ```
  Label plan. PE loopbacks are 10.255.0.{1,2,3}. A PE adjacent to its peer
  needs no transport label (implicit-null / PHP), so a copy pe1→pe2 is a
  single-LSE frame: just the BUM service label. pe2↔pe3 are two hops apart, so
  those copies carry a transport label (13 toward pe3, 12 toward pe2) that pe1
  pops and forwards — the classic pop-and-forward: the service label
  underneath belongs to the *egress* PE and pe1 must never look it up locally.
  `mpls_pop` on pe1 is what proves that transit hop actually happened.

  No unicast FDB anywhere: replies ride the unknown-unicast flood (the "U" in
  BUM), so every c*↔c* pair reaching each other proves replication, service-
  label decap + flood, and split horizon (a horizon leak would loop BUM
  between pe2 and pe3 forever).

  Scenario: Flood one bridge domain across three PEs over MPLS
    ...
    And the cradle stat "mpls_l2_bum" in namespace "pe2" via gRPC as "ctl2" should be nonzero
    ...
    And the cradle stat "mpls_pop" in namespace "pe1" via gRPC as "ctl1" should be nonzero
