@serial
@cradle_vpws_vxlan_zebra
Feature: BGP EVPN VPWS over VXLAN programs the eBPF E-Line
  The full EVPN VPWS provider edge (RFC 8214) over a VXLAN fabric
  (RFC 8365 §6), driven by zebra-rs and forwarded in eBPF: each PE's
  `vpws` service — under the DEFAULT `encapsulation vxlan`, no SRv6
  locator anywhere — advertises an Ethernet A-D per-EVI route (Type-1)
  carrying its service VNI in the label field, the VXLAN Encapsulation
  extended community, and the loopback VTEP as next hop. Importing the
  peer's Type-1 drives one cradle AddXconnect that binds the E-Line both
  ways: the AC's ingress XCONNECT encap toward the remote VTEP with the
  VNI the REMOTE advertised (downstream-assigned — eline1's VNIs are
  asymmetric on purpose), and the local E-Line VNI decap that emits raw
  on the same AC, plus SetVtepSource from the advertised next hop.

  The VTEPs are the loopbacks (BGP peers over them, so the advertised
  next hop IS the VTEP); the static /32 routes toward them resolve the
  encap's FIB4 lookup, and the underlay ARP the BGP session performs
  seeds NEIGH4 — everything the datapath needs arrives over the tee,
  zero static state in cradle.

  Topology (kernel v4+v6 forwarding off on the PEs):
  ```
   c1 ── pe1[cradle+zebra] ──10.0.12.0/24── pe2[cradle+zebra] ── c2
   10.0.0.1  VTEP 192.0.2.1 (lo)  |  VTEP 192.0.2.2 (lo)   10.0.0.2
      vpws eline1: evi 100, pe1 svc-id 101 (vni 5001) ⇄ pe2 svc-id 102
  ```

  Scenario: Cross-connect two CEs through a BGP-signalled VXLAN E-Line
  ...
  Scenario: A VLAN-scoped E-Line multiplexes the same ACs by 802.1Q VID
    ...
    # shares pe1c/pe2c with the untagged eline1: the AC demuxes by tag —
    # VID-30 frames ride the VLAN-scoped service (the tag crossing
    # transparently, demuxed at the egress from the DX2V table), untagged
    # traffic still rides eline1.
    #
