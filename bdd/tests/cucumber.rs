...
impl World {
    ...
    /// one's — for `cradle_evpn_vxlan`, those of `cradle_evpn_vxlan_bum` and
    ...
    /// damage: the startup sweep deletes the sibling's *live* namespaces, and
    /// the clean-environment check reports them as this feature's leaked ones.
    /// Bridges and host veths are unaffected — they carry `short_id()`, a hash
    /// of the whole tag.
...
// ----------------------- cradle data-plane steps --------------------------

/// Resolve the cradle binary (overridable via $CRADLE).
fn cradle_bin() -> String {
    std::env::var("CRADLE")
        .unwrap_or_else(|_| format!("{}/../target/debug/cradle", env!("CARGO_MANIFEST_DIR")))
}

#[when(expr = "I disable IPv4 forwarding in namespace {string}")]
async fn disable_ipv4_forwarding(world: &mut World, namespace: String) {
    let scoped = world.ns(&namespace);
    netns::exec_in_netns(&scoped, "sysctl", &["-wq", "net.ipv4.ip_forward=0"])
        .await
        .expect("Failed to disable IPv4 forwarding");
    println!("✓ disabled IPv4 forwarding in namespace {}", scoped);
}
...
/// Start cradle in a namespace with a JSON config from
/// `tests/configs/<feature>/<config>`. cradle loads + attaches the eBPF data
/// plane, then writes its pid file — we poll for that before returning.
#[when(expr = "I start cradle in namespace {string} with config {string}")]
async fn start_cradle(world: &mut World, namespace: String, config: String) {
    let scoped = world.ns(&namespace);
    let pid_file = world.pid_file(&namespace);
    ...
    netns::spawn_in_netns_env(
        ...
        &[("RUST_LOG", "info")],
        &cradle,
        &["serve", "--config", &config_path, "--pid-file", &pid_file],
    )
    .await
...
async fn stop_cradle(world: &mut World, namespace: String) {
    if keep_topology() {
        println!(
            "⏭  BDD_KEEP set — leaving cradle running in namespace {}",
            world.ns(&namespace)
        );
        return;
    }
...
async fn test_topology_exists(world: &mut World) {
    // Generic: any namespace scoped to this feature means the topology is up.
    let prefix = format!("{}_", world.feature_tag);
    world.topology_running = !netns::list_netns_with_prefix(&prefix)
        .await
        .unwrap_or_default()
        .is_empty();
}
